...
namespace parametric_shapes
{
	...
	//!         data
	bonobo::mesh_data createQuad(float width, float height, unsigned int resw, unsigned int resh);

	//! \brief Create a box consisting of 12 triangles and make it
	//!        available to OpenGL.
	//!
	//! @param side the side of the cube
	//! @return wrapper around OpenGL objects' name containing the geometry
	//!         data
	bonobo::mesh_data createCube(float side);
